...
var resolver = function (obj, path, depth, callback, upsert) {
    ...
    for (i = 0; i < depth; i++) {
        ...
        if("object" === typeof obj) {
            ...
            err = new TypeError(GETTER_ERROR.replace('%s', key) + typeof obj);
            break;
        }
    }
    if(callback) {
...
ptp.update = function (query, value, callback, upsert) {
    ...
    var path = query && query.split(this.delim) || [],
        ...
        set = function (obj, key, val) {
            return "object" === typeof obj && key ? obj[key] = val :
                new TypeError(SETTER_ERROR.replace('%s', key) + typeof obj);
        };

    if(callback) {
        resolver(ctx, path, depth, function (err, obj) {
            err || (result = set(obj, lastKey, value)) instanceof Error && (err = result);
 ...
 * @param {String} query Query string.
 * @returns {Object|Boolean} Result resolving copy or false.
 */
