/**
 * @fileOverview
 * Object Manager class provides easy to use, chaining / async-callback API (getter, setter)
 ...
 * ObjectManager methods support async style callback calls with proper error hanling suppert.
 ...
 * exampleObject = {},
 * manager = new ObjectManager(exampleObject);
 *
 * // call with callback (errors handling)
 * manager.update("a.b.0.c", 10, function (err, obj) {
 ...
 */
var ObjectManager = module.exports = function (ctx, delim) {
 ...
 */
var manager = function (obj, path, depth, callback, upsert) {
    var err = null, i, key;
 ...
 * @private
 */
var ptp = ObjectManager.prototype;
...
ptp.find = function (query, callback) {
    ...
    var path = query && query.split(this.delim) || [],
        result = manager(this.ctx, path, path.length, callback);
...
ptp.update = function (query, value, callback, upsert) {
    ...
    var path = query && query.split(this.delim) || [],
        ...
        ctx = this.ctx,
        result = callback || manager(ctx, path, depth, callback, upsert),
    ...
    if(callback) {
        manager(ctx, path, depth, function (err, obj) {
